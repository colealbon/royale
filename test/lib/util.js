...
import {decryptPGPMessage} from '../../src/lib/util.js';
...
-----END PGP MESSAGE-----
    ...
    test('handlePost throws error on no content', function() {
        return handlePost()
        .catch(err => assert.equal(err.toString(), 'Error: missing content'));
    });
    ...
    test('handlePost happy path unclassified fake data', function() {
        return handlePost('bubblegum')(openpgp)()()
        .then(response => assert.equal(response, 'bubblegum'))
        .catch(err => assert.equal(err.toString(), 'this code path should be dead'));
    });
...
G7Pw9EktJ8t0DvuKMjl9CsI7cY6BDXs3Jn4J
=rfvU
-----END PGP MESSAGE-----`)(openpgp)(localStorage)()
